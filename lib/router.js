...
class Router {
  ...
  async handle(req, res) {
    ...
    // Vercel rewrite destinations can double the /api prefix (see vercel.json).
    // Normalise so route matching always sees the original request path.
    if (parsed.pathname === '/api/api') parsed.pathname = '/api';
    else if (parsed.pathname.startsWith('/api/api/')) parsed.pathname = parsed.pathname.slice(4);
